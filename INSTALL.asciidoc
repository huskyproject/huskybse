...
called `perl-Module-Build`. To configure the build using universal makefiles,
use the file *huskymak.cfg*.
...
As a result, the `~/husky` directory will be created. Husky sources will be
downloaded to this directory in the future. But now we are interested in the
file `huskymak.cfg` lying in this directory. This is the very build
configuration file that you need to view and, possibly, change something in it.

These actions need to be performed only once. Now you can start the build:

  ./build.sh

After the build is complete, to install the built programs, run

  sudo make -j install

Here, the `-j` option sets the parallel execution. If you want to uninstall
the installed programs, run
